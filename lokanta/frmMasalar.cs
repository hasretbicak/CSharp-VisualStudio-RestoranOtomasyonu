            ...
            while (dr.Read())
            {
                foreach(Control item in this.Controls)
                {
                    ...
                    if( item is Button)
                    {
                        ...
                        {
                            item.BackgroundImage = (System.Drawing.Image)(Properties.Resources.green);
                        }
                        else if (item.Name == "btnMasa" + dr["id"].ToString() && dr["durum"].ToString() == "2")
                        {
                            ...
                            fark.Days > 0 ? string.Format("{0} Gün ", fark.Days) : "",
                             fark.Hours > 0 ? string.Format("{0} Hours ", fark.Hours) : "",
                            fark.Minutes > 0 ? string.Format("{0} Minutes ", fark.Minutes) : "").Trim() + "\n\n\nMasa" + dr["id"].ToString();

                            item.BackgroundImage = (System.Drawing.Image)Properties.Resources.red;


                        }
                        else if (item.Name == "btnMasa" + dr["id"].ToString() && dr["durum"].ToString() == "3")
                        {
                            item.BackgroundImage = (System.Drawing.Image)(Properties.Resources.blue);
                        }
                        else if (item.Name == "btnMasa" + dr["id"].ToString() && dr["durum"].ToString() == "4")
                        {
                            item.BackgroundImage = (System.Drawing.Image)(Properties.Resources.yellow);
                        }
